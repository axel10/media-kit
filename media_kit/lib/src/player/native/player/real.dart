...
class NativePlayer extends PlatformPlayer {
  ...
  Future<NativePropertyResult> setProperty(
    ...
    Future<NativePropertyResult> function() async {
      if (disposed) {
        throw AssertionError('[Player] has been disposed');
      }
      ...
      final code = await _setPropertyStringWithResult(property, value);
      return NativePropertyResult(
        property,
        value,
        code,
        error: _errorString(code),
      );
    }
  ...
  /// Sets multiple properties for the internal libmpv instance of this [Player].
  Future<List<NativePropertyResult>> setProperties(
    Map<String, String> properties, {
    bool waitForInitialization = true,
    bool synchronized = true,
  }) async {
    Future<List<NativePropertyResult>> function() async {
      final results = <NativePropertyResult>[];
  ...
  Future<void> _create() {
    return lock.synchronized(() async {
      final startupResults = <NativePropertyResult>[];
      try {
        ...
        for (final entry in configuration.nativeInitialProperties.entries) {
          final code = await _setPropertyStringWithResult(
            entry.key,
            entry.value,
          );
          startupResults.add(
        ...
        if (!_startupPropertyResultsCompleter.isCompleted) {
          _startupPropertyResultsCompleter.complete(
            List.unmodifiable(startupResults),
          );
        }
  ...
  String? _errorString(int code) {
    if (code < 0) {
      return mpv.mpv_error_string(code).cast<Utf8>().toDartString();
    }
    return null;
  }
  ...
  Future<int> _setPropertyWithResult(
    String name,
    int format,
    Pointer<Void> data,
  ) async {
    ...
    if (configuration.async) {
      ...
      final code = mpv.mpv_set_property(ctx, namePtr.cast(), format, data);
      calloc.free(namePtr);
      _logError(code, '_setProperty($name, $format)');
      return code;
    }
  }

  Future<void> _setProperty(String name, int format, Pointer<Void> data) async {
    await _setPropertyWithResult(name, format, data);
  }
  ...
  Future<void> _setPropertyString(String name, String value) async {
    await _setPropertyStringWithResult(name, value);
  }

  Future<int> _setPropertyStringWithResult(String name, String value) async {
    final string = value.toNativeUtf8();
    // API requires char**.
    final ptr = calloc<Pointer<Void>>(1);
    ptr.value = Pointer.fromAddress(string.address);
    final code = await _setPropertyWithResult(
      name,
      generated.mpv_format.MPV_FORMAT_STRING,
      ptr.cast(),
    );
  ...
  /// [Future] that resolves with startup property application result(s).
  Future<List<NativePropertyResult>> get waitForNativeInitialProperties =>
      _startupPropertyResultsCompleter.future;

  final Completer<List<NativePropertyResult>> _startupPropertyResultsCompleter =
      Completer<List<NativePropertyResult>>();
